...
## Parallel workflow — deployment and qualified reporting

- Add deployment identity/reference: `deployments/<deployment_id>/deployment.json` (`audiollm.deployment.v1`, `source_manifest_sha256` copied to `metadata.json.source.deployed_source_sha256`); a registry rebuild cannot invent a missing deployment hash
- Enforce `output_model/<campaign>/<modality>/<dataset>/<run_name>/fold_<n>` layout (importer derives modality/dataset from path position; do not shorten)
- Require `evaluation.evaluation_view` for production reportability (e.g., `harmonized_all_windows_full_coverage` for harmonized DAIC with `original_teacher_forced` and `subject` aggregation); submission fails closed on null view
- Require a new attempt ID for every rerun/redeployment; link retries via `metadata.json.supersedes_attempt_id` and preserve failed attempts
- Use group-scoped `exp compare --group <id> --attempts <csv> --dataset ... --metric ... --namespace ... --backend ... --view ... --aggregation ...` or deterministic `group_report` for winner selection; global `exp best` alone is not group-scoped and may return unrelated historical runs
