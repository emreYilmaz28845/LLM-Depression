...
- macro-F1 selection and early stopping for harmonized configs, mode `max`;
...
- harmonized audio examples contain one chunk per prompt; DAIC uses participant-only packed30 all-chunk coverage with subject-normalized weights;
...
## Parallel workflow — additional validation

- Verify pin, deployment immutability, common-path resolution (train/eval/checkpoint/fold/context/log paths match), output-layout `campaign/modality/dataset/run/fold`, collection filter (compact `standalone_eval` preserved while adapters excluded), stacked-branch ancestry (parent SHA), and final auditor (`tools/audit_parallel_workflow_implementation.py --state <state.json> [--allow-incomplete] --output <audit.json>`)
- Require full `python -m pytest tests/` for workflow contract changes (e.g., submission wrapper, deployment, collection, comparison, or skill/doc contract changes)
