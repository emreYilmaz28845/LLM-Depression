...
## Parallel workflow — lane journaling

- Require lane grant (wording, scope, budgets, expiry, exclusions), worktree, branch ancestry (parent branch/SHA), deployment ID/hash, attempt/job IDs, phase state (`state.json` current_phase and phase statuses), and integration decision (Git/semantic conflicts, winner selection) when available
- Distinguish progress entries (Phase N passed) from terminal `COMPLETE` (Phase 13 auditor passed); never claim task completion before the terminal auditor passes
