#!/usr/bin/env python3
"""Managed orchestration for the Turkish pooled question-condition campaign."""
...
def _lane_and_deployment(slug: str, deployment_id: str | None, *, execute: bool) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    if pin.get("parent_sha") != "d74dbe35da92e310b89ae91df0556f37b1c03f76":
...
def _preflight_pairs(audit: dict[str, Any]) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    for pair in audit.get("pairs") or []:
        ...
        result[key] = pair
    required = {("pooled", "native"), ("pooled", "english")}
...
def _head_config(*, fold_job: dict[str, Any], method: str, backend: str, trials: int) -> dict[str, Any]:
    ...
    recording = str(fold_job["recording_condition"])
    if recording != "pooled":
        raise CampaignError(f"pooled head config requires recording_condition pooled, got {recording!r}")
    return {
...
def _preflight_script(deployment: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "retry=1",
        "while [ -e \"$audit\" ]; do retry=$((retry + 1)); audit=\"$audit_dir/audit_retry_${retry}.json\"; done",
        "python scripts/turkish_pooled_qcond_preflight.py"
        f" --stage production --manifest-root {q(REMOTE_RUNTIME_ROOT / 'manifests')}"
        f" --split-root {q(REMOTE_RUNTIME_ROOT / 'splits')}"
...
for path in sorted(root.glob('audit*.json')):
    ...
    if payload.get('status') == 'passed' and payload.get('group_id') == {GROUP_ID!r}:
        passed.append(path)
if not passed:
    raise SystemExit('no passed Turkish pooled preflight audit')
print(passed[-1])
PY
"""
...
def _run_preflight(args: argparse.Namespace) -> int:
    if args.local:
        if args.manifest_root is None or args.split_root is None:
            raise CampaignError("local preflight requires --manifest-root and --split-root")
        command = [
            sys.executable,
            str(PROJECT_ROOT / "scripts/turkish_pooled_qcond_preflight.py"),
            "--stage", args.stage,
...
def _remote_submission_script(plan: dict[str, Any], deployment: dict[str, Any]) -> str:
    ...
    for index, item in by_backbone.items():
        ...
        lines.append(
            ...
            f" --reason {q('Turkish pooled LogReg submitted')}"
        )
        ...
        lines.append(
            ...
            f" --reason {q('Turkish pooled Optuna submitted')}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    preflight.add_argument("slug", nargs="?", default="exp-turkish-pooled-qcond-v1")
    ...
    preflight.add_argument("--manifest-root", type=Path)
    preflight.add_argument("--split-root", type=Path)
