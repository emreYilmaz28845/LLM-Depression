#!/usr/bin/env python3
"""Lifecycle and evidence CLI for Turkish pooled hidden-head attempts."""
...
from src.turkish_pooled_qcond_tracking import (
    finish_head_attempt,
    initialize_turkish_pooled_head_attempt,
    materialize_job_evidence,
    materialize_turkish_pooled_head_evidence,
    record_head_job,
...
def parse_args() -> argparse.Namespace:
    ...
    for name, function in (("validate", validate_head_attempt), ("validate-job", validate_job_attempt), ("finish", finish_head_attempt)):
...
def main() -> None:
    ...
    if args.command == "init":
        result = initialize_turkish_pooled_head_attempt(
            args.attempt_dir,
        ...
        result = record_head_job(
            args.attempt_dir, job_key=args.job_key, job_type=args.job_type,
            event_type=args.event_type, slurm_job_id=args.slurm_job_id,
        ...
        result = materialize_turkish_pooled_head_evidence(
            args.attempt_dir, predictions_path=args.predictions,
            metrics_path=args.metrics, checkpoint_path=args.checkpoint_path,
        )
    elif args.command == "job-materialize":
        result = materialize_job_evidence(args.attempt_dir, artifact_paths=args.artifact)
    else:
