...
LOG_ROOT="${LOG_ROOT:-$PROJECT_ROOT/experiment_runtime/turkish-pooled-qcond/logs/logreg}"
...
on_error() {
    ...
    "${WORKER[@]}" transition --attempt-dir "$ATTEMPT_DIR" --to-state FAILED \
        --reason "Turkish pooled question-conditioned LogReg worker failed" || true
    exit "$code"
}
trap on_error ERR

"${WORKER[@]}" transition --attempt-dir "$ATTEMPT_DIR" --to-state RUNNING --reason "pooled hidden extraction and LogReg worker started"
"${WORKER[@]}" record --attempt-dir "$ATTEMPT_DIR" --job-key head --job-type hidden_extraction \
...
echo "Turkish pooled question-conditioned LogReg worker completed: attempt=$ATTEMPT_DIR"
