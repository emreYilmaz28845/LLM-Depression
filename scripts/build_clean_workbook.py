...
def _build_turkish_pooled_lookup(report_path: Path | None = None) -> None:
    """Populate TURKISH_POOLED_MIXED_LOOKUP from the pooled report.

    Headline convention (locked 2026-09-07): Turkish is a train_val CV, so the
    headline cells show the seed-1337 five-fold mean (fold-mean) of the combined
    ...
    if len(rows) != 30:
        raise ValueError(f"Turkish pooled report seed-1337 rows has {len(rows)} route rows; expected 30")
