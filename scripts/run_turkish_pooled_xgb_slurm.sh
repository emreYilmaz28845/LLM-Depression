...
LOG_ROOT="${LOG_ROOT:-$PROJECT_ROOT/experiment_runtime/tpooled_qcond/logs/xgb}"
...
on_error() {
    ...
    "${WORKER[@]}" transition --attempt-dir "$ATTEMPT_DIR" --to-state FAILED \
        --reason "Turkish pooled Optuna worker failed" || true
    exit "$code"
}
trap on_error ERR
...
echo "Turkish pooled XGBoost worker completed: attempt=$ATTEMPT_DIR trials=$TRIALS"
