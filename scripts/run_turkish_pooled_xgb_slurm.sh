...
LOG_ROOT="${LOG_ROOT:-$PROJECT_ROOT/experiment_runtime/turkish-pooled-qcond/logs/xgb}"
...
WORKER=(python "$PROJECT_ROOT/tools/turkish_pooled_qcond_worker.py")
...
on_error() {
    ...
    "${WORKER[@]}" transition --attempt-dir "$ATTEMPT_DIR" --to-state FAILED \
        --reason "Turkish pooled question-conditioned Optuna worker failed" || true
    exit "$code"
}
trap on_error ERR

"${WORKER[@]}" transition --attempt-dir "$ATTEMPT_DIR" --to-state RUNNING --reason "pooled Optuna XGBoost worker started"
"${WORKER[@]}" record --attempt-dir "$ATTEMPT_DIR" --job-key head --job-type hidden_classifier \
    --event-type STARTED --slurm-job-id "${SLURM_JOB_ID:-}" --status RUNNING

export TURKISH_POOLED_QCOND_STAGE="$STAGE"
export TURKISH_POOLED_QCOND_ATTEMPT_DIR="$ATTEMPT_DIR"
...
echo "Turkish pooled question-conditioned XGBoost worker completed: attempt=$ATTEMPT_DIR trials=$TRIALS"
