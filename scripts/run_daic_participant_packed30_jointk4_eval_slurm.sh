...
# best_model; last_model must never be substituted. Production runs fp32 to
# match the canonical comparison; smokes run bf16.
...
echo "== packed30 jointk4 official-test evaluation =="
echo "checkpoint=$BEST_DIR config=$CONFIG dtype=$INFERENCE_DTYPE"
printf 'Launch: '; printf '%q ' "${CMD[@]}"; printf '\n'
"${CMD[@]}"
echo "== packed30 jointk4 evaluation finished =="
