...
- One common resolved override array is used for train, evaluation, manifest, checkpoint, and sidecar paths; `scripts/submit_train_and_eval.sh` loads YAML with those overrides before calculating `FOLD_DIR`, `CHECKPOINT_DIR`, etc., and passes identical overrides to `run_train_slurm.sh`/`run_eval_slurm.sh`
