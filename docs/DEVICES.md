...
Project GPU policy, updated 2026-08-18: no project-wide GPU cap. The professor explicitly waived the earlier 64-H100 ceiling; run as many jobs in parallel as the scheduler, account, and QoS will grant. Launchers default to one lane per task (one four-GPU training lane per training job, one one-GPU lane per auxiliary job) so the whole matrix runs at once. Tune `MAX_CONCURRENT_TRAINS` / `MAX_CONCURRENT_AUX` (or `MAX_CONCURRENT_POSTPROCESS`) when a plan needs different concurrency. This policy does not change the per-job resource shapes in the table above.
...
Treat the Slurm headers as defaults, not timeless facts. Before submission, inspect the selected scripts and verify account, QoS/partition, per-job GPU count, concurrency lane counts, wall time, dependency chain, config, folds, and run name.
...
Job submission and cancellation, result retrieval from GPFS, and W&B cloud export (evidence-first, hash-verified) are pre-authorized; training and evaluation run only via Slurm, never directly on a transfer or login node. An agent may perform relevant read-only inspection when needed. Explicit user authorization is still required before actions such as:
...
Per-task autonomy grants: the user may grant full autonomy for one named task in chat. The agent journals the grant (wording, scope, expiry condition, exclusions) before the first mutation. The grant covers only that task and ends on completion, source-SHA change, hard stop, or user revocation. Destructive sync, deletions, evidence overwrite, experiment expansion, package installs, and shared-environment changes stay excluded unless the grant names them.
...
## Parallel workflow — isolated deployments and runtime (Phases 0-13)

New workflow (implemented) uses `~/worktrees/LLM-Depression-*` for lanes, `tools/exp.py create`/`deploy`/`submit`/`status`/`collect`/`validate`/`compare`/`finish`, and `tools/check_worktree_pin.py` for pin verification. Source deployments are immutable under `deployments/<id>/code` with `deployment.json`; writable runtime under `experiment_runtime/<id>/` (contexts, manifests, splits, logs); permanent outputs under `LLM-Depression/output_model/<campaign>/<modality>/<dataset>/<run_name>/fold_<n>` (importer-supported). Writable-path contract: `output_dirs.manifest_dir = <runtime>/manifests/<dataset>`, `output_dirs.split_dir = <runtime>/splits/<dataset>`, `output_dirs.run_root = <permanent>/output_model/<campaign>/<modality>/<dataset>`, `LOG_ROOT = <runtime>/logs/<job_type>/<dataset>`, `EXPERIMENT_CONTEXT = <runtime>/contexts/<attempt_id>/fold_<n>/context.json`. Use `sbatch --chdir="$PROJECT_ROOT"` so the deployment path overrides the fixed worker `#SBATCH --chdir`. See `docs/PARALLEL_EXPERIMENT_WORKFLOW_PLAN.md` Phases 0-13 and `tools/audit_parallel_workflow_implementation.py`.
