...
For GPU work, size the launcher's concurrency lanes for the whole matrix. There is no project-wide GPU cap; run as many jobs in parallel as the scheduler, account, and QoS will grant. The scheduler may still grant fewer GPUs than requested, and this does not change a job's configured GPU shape.
...
[ ] Size concurrency lanes so every eligible job runs in parallel (no project GPU cap)
...
## Parallel workflow — isolated deployment lifecycle (Phases 0-13)

For code-changing experiments, use the managed lane workflow instead of the legacy mutable-checkout `bash scripts/sync_to_cluster.sh` + `CONFIG=... bash scripts/submit_train_and_eval.sh` on the permanent checkout.

1. Create lane: `python tools/exp.py create <slug> --tier {1,2} [--from <branch-or-sha>] [--dry-run]` → branch `agent/exp-*` or `agent/feat-*` under `~/worktrees/`, definition under `experiments/definitions/`, ignored `.agent-pin.json`
2. Verify pin before any edit/deploy/submission: `python tools/check_worktree_pin.py`
3. Deploy: `python tools/exp.py deploy <slug> --dry-run` then `--execute` (new-target-only rsync via `transfer1`, `--chdir="$PROJECT_ROOT"`, no `--delete`, `deployment.json` outside `code/`, `experiment_runtime/<id>/` writable)
4. Submit: `CONFIG=... RUN_NAME=... FOLD=0 EXTRA_TRAIN_ARGS="--set ..." EXTRA_EVAL_ARGS="--set ..." LOG_ROOT=... EXPERIMENT_CONTEXT=... bash scripts/submit_train_and_eval.sh` (common overrides, isolated `run_root`/`manifest_dir`/`split_dir`/`LOG_ROOT`/`EXPERIMENT_CONTEXT`, collision and `evaluation_view` checks, 4-GPU DDP train + 1-GPU eval)
5. Monitor: `python tools/exp.py status <slug>` + `squeue`/`sacct` (terminal `COMPLETED` with `ExitCode=0:0` is authoritative; empty `squeue` is not success)
6. Collect: `python tools/exp.py collect <slug> --dry-run` then `--execute` (include compact evidence before excluding `best_model/***`/`last_model/***`, preserve `best_model/standalone_eval/***`)
7. Validate: `python tools/exp.py validate <slug>` (hash, metric recomputation, qualifier checks, `SYNCED_LOCALLY` → `LOCALLY_VALIDATED` → `REPORTABLE`)
8. Compare: `python tools/exp.py compare --group <id> --attempts <csv> --dataset ... --metric ... --namespace ... --backend ... --view ... --aggregation ...` (group-scoped, full qualifiers)

See `docs/PARALLEL_EXPERIMENT_WORKFLOW_PLAN.md` for the full 14-phase runbook, `outputs/parallel_workflow_implementation/<execution_id>/state.json` for the execution ledger, and `tools/audit_parallel_workflow_implementation.py` for the final auditor.
