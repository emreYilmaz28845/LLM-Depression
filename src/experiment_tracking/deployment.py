...
def _is_generated_python_cache(path: str) -> bool:
    """Return whether a remote extra is interpreter-generated bytecode.

    CPython writes bytecode atomically through temporary files beside the final
    name (``module.cpython-310.pyc.140103725005744``), so the temporary form must
    be tolerated too: a killed import would otherwise leave a file that makes
    every later deployment verification fail on "unexpected files".
    """
    ...
    if "__pycache__" not in parts:
        return False
    name = Path(path).name
    return ".pyc" in name or ".pyo" in name
...
def verify_remote_tree(
    ...
    ``.provenance/**`` and interpreter-generated ``__pycache__/*.pyc`` files).
    """
    ...
    allowed_extras = {
        ...
        or _is_generated_python_cache(p)
    }
