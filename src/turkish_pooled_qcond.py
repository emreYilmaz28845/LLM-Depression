"""Typed matrix and deterministic plan builder for the Turkish pooled study.

The pos_only-versus-negative_only campaign has a different scope and remains
isolated.  This module is the single source of truth for the pooled
question-condition study: ten backbone cells trained on all 2,221 windows
with a frozen per-example tag, three training seeds, five folds, and one
hidden-state LogReg plus one Optuna-100 route per fresh backbone fold.
"""
...
GROUP_ID = "turkish-pooled-qcond-v1-20260903"
EXPERIMENT_ID = "exp-turkish-pooled-qcond-v1-20260903"
...
GROUP_RELATIVE_PATH = (
    ...
    "turkish-pooled-qcond-v1-20260903.yaml"
)
...
class BackboneCell:
    ...
    def campaign(self) -> str:
        return f"turkish_pooled_qcond_v1_{self.backbone}_{self.transcript_token}"
...
def load_cells(repo_root: Path | None = None) -> tuple[BackboneCell, ...]:
    """Load and validate the exact ten pooled group cells."""

    group = _load_group(repo_root)
    records = group.get("scope", {}).get("backbone_cells")
    if not isinstance(records, list) or len(records) != 10:
        raise MatrixError(f"campaign group must contain exactly 10 pooled backbone cells, got {records!r}")
    ...
    for raw in records:
        ...
        if cell.recording_condition not in {"pooled"}:
        ...
        cells.append(cell)
    if {cell.cell_id for cell in cells} != {f"Q{i:02d}" for i in range(1, 6)} | {f"G{i:02d}" for i in range(1, 6)}:
        raise MatrixError("campaign cell IDs are not the locked Q01-Q05/G01-G05 set")
    if sum(cell.recording_condition == "pooled" for cell in cells) != 10:
        raise MatrixError("locked matrix must contain ten pooled cells")
    return tuple(cells)
...
def _fold_identity(cell: BackboneCell, seed: int, fold: int, source_sha: str) -> FoldIdentity:
    ...
    run_name = (
        f"tpooled_qcond_v1_{cell.backbone}_{language}_{cell.modality}"
    ...
    # transcript language. Both pooled manifests are prebuilt by
    # scripts/build_turkish_pooled_manifest.py (native 2,221 rows +
    # EN 2,221 rows, byte-identical splits adopted). The model-specific run
    # root remains isolated.
    path_token = "pooled_en" if language == "english" else "pooled"
...
def _config_summary(repo_root: Path, cell: BackboneCell) -> dict[str, Any]:
    ...
    return {
        ...
        "dataset_variant": config.get("dataset_variant", "pooled_t17"),
